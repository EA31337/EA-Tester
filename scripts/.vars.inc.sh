...
is_vm && set -x
TERMINAL_EXE="$(find "$ROOT" "$OPT" "$HOME" "$WINE_PATH" -not -path "*/WebInstall/*" -name terminal.exe -print -quit)"
...
TERMINAL5_EXE="$(find "$ROOT" "$OPT" "$HOME" "$WINE_PATH" -not -path "*/WebInstall/*" -name terminal64.exe -print -quit)"
