...
echo "Checking secur32.dll.so..." >&2
if [ -d "/usr/lib/i386-linux-gnu" ]; then
  ...
  if [[ -n $(strings "$CHANNEL_PATH" 2> /dev/null | grep 'libgnutls.so.26') ]]; then
    echo "Found that Wine's secur32 channel is using libgnutls26 at '$CHANNEL_PATH'"
    sudo apt-get -qy install libgnutls28:i386
    printf 'Overwriting channel with the new build for wine-1.8.0 and libgnutls28...' >&2
